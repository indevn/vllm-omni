...
async def send_text(ws, text: str, simulate_stt: bool, stt_delay: float) -> None:
    if simulate_stt:
        words = text.split(" ")
...
async def stream_tts(args) -> None:
    ...
    async with websockets.connect(args.url, max_size=64 * 1024 * 1024, proxy=None) as ws:
